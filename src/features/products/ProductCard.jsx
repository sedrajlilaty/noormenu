...
return(
<div className="flex bg-white rounded-xl overflow-hidden shadow-sm min-h-[130px]">
        <img
        src={product.image}
...
<p className="text-sm text-gray-400">
  ...
  {hasDiscount && (
    <span className="text-gray-400 text-sm line-through">{product.priceOld}</span>
           ...
           {product.hasOptions ? (
        <button onClick={() => onOpenOptions(product)} className="px-4 py-1.5 rounded-full border border-main text-main text-sm font-bold">
          اختر
        </button>
      ) : (
